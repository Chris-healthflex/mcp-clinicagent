...
class MedicalDatabase:
    ...
    def search_knowledge(self, query: str, top_k: int = 5) -> dict:
        ...
        # CACHE DISABLED - Always perform fresh searches
        ...
        try:
            ...
            # Uncomment below for metadata debugging if needed
            # if metadatas and len(metadatas) > 0:
            #     logger.info(f"Metadata sample: {metadatas[0]}")
            #     logger.info(f"Metadata keys: {list(metadatas[0].keys())}")
            ...
            for i, (doc, metadata, distance) in enumerate(zip(documents, metadatas, distances)):
                ...
                # Handle different metadata structures with better fallbacks
                # Try multiple possible keys for filename (order matters!)
                filename = None
                for key in ['file_name', 'filename', 'source', 'file', 'doc_name', 'document', 'name']:
                ...
                # If still no filename, check if metadata is empty
                if not filename:
                    if metadata:
                        # Try doc_id or document_id as fallback
                        filename = metadata.get('doc_id', metadata.get('document_id', 'no_source'))
                    else:
                        filename = 'empty_metadata'
                
                # Try multiple possible keys for chunk_id (order matters!)
                chunk_id = None
                for key in ['chunk_id', 'chunk', 'chunk_number', 'index', 'doc_id', 'id']:
                    if key in metadata and metadata[key] is not None:
                        chunk_id = metadata[key]
                        # If it's a UUID/long ID, shorten it
                        if isinstance(chunk_id, str) and len(chunk_id) > 15:
                            chunk_id = chunk_id[:8]  # Take first 8 chars of UUID
                        break
                ...
                # Debug logging disabled for cleaner output
                # if i == 0:
                #     logger.info(f"First result - filename: {filename}, chunk_id: {chunk_id}")
            ...
            logger.info(f"✓ Found {len(search_results)} results (CACHE DISABLED)")
